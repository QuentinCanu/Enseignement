...
\item Parmi les beignets à l'ananas, $35\%$ sont sans gluten; tandis que parmi les beignets à la pomme, $45\%$ sont sans gluten. 
\end{itemize}
...
\item $G$ : \og le beignet choisi est sans gluten \fg 
\end{itemize}
\begin{parts}
\part[1] Donner $P_A(G)$ d'après le contexte donné dans l'énoncé.
...
\node (B1B2) at (5,2.5) {$G$};
\node (B1R2) at (5,0.5) {$\overbar{G}$};
\node (R1B2) at (5,-0.5) {$G$};
\node (R1R2) at (5,-2.5) {$\overbar{G}$};
...
\end{tikzpicture}
\end{center}
\part[1] Calculer la probabilité $P(A \cap G)$
\part[2] Montrer que $P(G) = 0,42$
\part[2] Calculer la probabilité que le beignet soit à l'ananas, sachant qu'il est sans gluten.
\end{parts}
\end{questions}
\end{document}
...
\begin{parts}
\part[2] $1 + 3 + 3^2 + \dots + 3^{10}$
\part[2] $1 + 1,2 + 1,2^2 + \dots + 1,2^{25}$
\part[2] $1 + 0,9 + 0,9^2 + \dots + 0,9^{34}$
\end{parts}
...
\end{tikzpicture}
\end{center}
\part[2] En déduire le maximum de la fonction, et pour quel $x$ ce maximum est atteint.
\end{parts}
...
\part[1] Soit $(w_n)$ la suite définie, pour tout $n \in \N$,
\begin{equation*}
w_n = u_n - 6
\end{equation*}
\part[1] Calculer $w_0$.
\part[2] Montrer que la suite $(w_n)$ est géométrique de raison $7$.
\part[0,5] En déduire la formule explicite de $w_n$ en fonction de $n$.

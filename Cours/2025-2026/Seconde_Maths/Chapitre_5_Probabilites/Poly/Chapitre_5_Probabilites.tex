...
\subsection{Univers}
\begin{definition}
Une \textbf{expérience aléatoire} est une expérience\dots
\begin{itemize}
\item dont les résultats \textbf{possibles} sont connus;
\item mais dont le résultat \textbf{obtenu} n'est pas prévisible.
\end{itemize}
\end{definition}
...
\label{dice}
\item On tire une carte dans un jeu de 52 cartes, et on regarde la couleur (\textbf{Coeur},\textbf{Carreau},\textbf{Pique},\textbf{Trèfle}) obtenue. 
\label{cards}
\end{enumerate}
\end{example}
\begin{definition}
\hfill
\begin{itemize}
\item L'un des résultats possible d'une expérience aléatoire est appelé \textbf{issue}.
\item L'\textbf{univers} d'une expérience aléatoire est l'ensemble de ses issues.
\end{itemize}
\end{definition}
\begin{example}
Pour définir l'univers (fini) d'une expérience aléatoire, on met entre accolades toutes ses issues. On appelle cet univers $\Omega$ qui se lit \og Oméga \fg.
\begin{enumerate}
\item L'univers de l'experience \ref{dice} est $\Omega = \{1;2;3;4;5;6\}$ 
\item L'univers de l'experience \ref{cards} est $\Omega = \{\textbf{Coeur};\textbf{Carreau};\textbf{Pique};\textbf{Trèfle}\}$ 
\end{enumerate}    
\end{example}
\begin{exercize}
...
\begin{definition}
Un \textbf{événement} d'une expérience aléatoire est un ensemble contenant tout ou partie des issues de l'expérience aléatoire.
\end{definition}
\begin{example}
\hfill
\begin{enumerate}
\item Soit $A = \{1;3;5\}$. C'est un événement de l'expérience \ref{dice} (lancer de dé) : il correspond à \og Obtenir un impair \fg.
\item Soit $B = \{\textbf{Pique};\textbf{Trèfle}\}$. C'est un événement de l'expérience \ref{cards} (tirage d'une carte) : il correspond à \og La carte tirée est noire \fg.
\end{enumerate}
\end{example}
\begin{remark}
\hfill
\begin{itemize}
\item Un événement \textbf{certain} est un événement qui contient toutes les issues de l'expérience.  
\item Un événement \textbf{impossible} est un événement qui ne contient aucun élément. On le note $\emptyset$.
\end{itemize}
\end{remark}
\begin{exercize}
Compléter le tableau suivant :
\begin{center}
\begin{tabular}{|p{0.25\textwidth}|p{0.3\textwidth}|p{0.2\textwidth}|p{0.2\textwidth}|}
\hline
...
\hfill
\begin{itemize}
\item La \textbf{réunion} de $A$ et de $B$, notée $A \cup B$, est l'événement contenant toutes les issues de $A$ ainsi que celles de $B$.
\item L'\textbf{intersection} de $A$ et $B$, notée $A \cap B$, est l'événement contenant les issues présentes à la fois dans $A$ et dans $B$ 
\end{itemize} 
\end{definition}
\begin{center}
...
\end{center}
\begin{definition}
Le \textbf{complémentaire} de $A$, noté $\overbar{A}$, est l'ensemble des élément de $\Omega$ qui ne sont pas des éléments de $A$.
\end{definition}
\begin{center}
\begin{tikzpicture}[scale=0.6]
...
\begin{definition}
Soit une expérience aléatoire dont l'univers est \textbf{fini} : il est de la forme
\begin{equation*}
\Omega = \left\{e_1; e_2; \dots; e_n \right\}, \text{ avec } n \geq 1.
\end{equation*}

Une \textbf{loi de probabilité} sur $\Omega$ est l'association de chaque issue $e_i$ à un nombre $p_i$ compris entre $0$ et $1$ inclus. De plus, la somme de tous ces nombres doit être égale à $1$.
\end{definition}
\begin{example}
...
\end{exercize}
\begin{definition}
On dit qu'une expérience aléatoire est en \textbf{situation d'équiprobabilité} si toutes les issues ont la même probabilité.
\end{definition}
\begin{example}
...
\begin{example}
On tire au sort une personne dans un lycée de $1000$ personnes. Sachant qu'il y a $195$ secondes, $235$ premières, $632$ filles dont $345$ en terminale et $155$ en première, compléter le tableau suivant et donner la probabilité de tomber sur un garçon en seconde.
\begin{center}
\resizebox{\textwidth}{!}{
...
\end{remark}
\begin{definition}
Soit une expérience aléatoire d'univers fini $\Omega$, et deux événements $A$ et $B$. Ces deux événements sont dits \textbf{incompatibles} si et seulement s'ils n'ont pas d'issues en commun. Autrement dit, si et seulement si $A \cap B = \emptyset$.
\end{definition}
\begin{proposition}
